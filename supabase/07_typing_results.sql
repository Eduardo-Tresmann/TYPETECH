...
create table if not exists public.typing_results (
  ...
  accuracy numeric not null check (accuracy >= 0), -- Precisão em porcentagem (sem limite máximo)
  ...
  verified boolean not null default true -- Marca resultados auditados/gerados pelo backend
);

-- Garantir que ambientes existentes possuam a coluna 'verified'
